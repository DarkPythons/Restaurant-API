...
async def update_data_bakcet(*, user_id, session, order_id):
...
async def get_active_order_fo_user(*, user_id, session):
...
async def get_order_by_id(*, order_id, session):
    query = select(OrderTable).where(OrderTable.c.id == order_id)
    result = await session.execute(query)
    return result.mappings().all()


async def delete_order_by_id(*, order_id, session):
