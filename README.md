<p align="center">
      <img src="https://i.ibb.co/0XcwvjC/photo.jpg" alt="PetProjectPhoto" border="0" width="727">
</p>

<p align="center">
   <img alt="Static Badge" src="https://img.shields.io/badge/Licencse-MIT-success">
   <img alt="PyPI - Status" src="https://img.shields.io/pypi/status/FastAPI">
   <img alt="PyPI - Python Version" src="https://img.shields.io/pypi/pyversions/FastAPI">
</p>

## About

In fact, this pet project is a consolidation of the FastAPI theme, it has logging, error handling, email mailing (enabled). The project was mainly based on the topic of food delivery, so it has restaurants, couriers, users and much more.

## Documentation

<h3>Base settings</h3>
<li>Download the project from github</li>
<li>Through the terminal, find and install all dependencies from the requirements file with the pip install -r command requirements.txt</li>
<li>Create and configure the file.env when using the file .env_example</li>
<h3>Launching the application</h3>
<li>After setting up the environment, you can launch the application</li>
<li>Go to the app directory via the terminal and enter the command: uvicorn main:app</li>
<li>Next, open the website in the browser http://127.0.0.1:8000/docs where will there be an interactive website with all API functions</li>
<li>After that, you can use the basic API functions, but for the courier service to work and assign couriers, you need to log into the database and assign a superuser user</li>
<li>Look at the file command_app.txt for advanced teams, including the inclusion of mailing lists</li>

<h3>When opening http://127.0.0.1:8000/docs
and when the application is running correctly, you will be greeted by a description</h3>
<img src="https://i.ibb.co/VLMRtzD/chrome-l-Rt-QZYo-SLE.png" alt="chrome-l-Rt-QZYo-SLE" border="0" width="800">
<p>After scrolling down, you will encounter all kinds of operations</p>
<img src="https://i.ibb.co/BL1pphm/API-exmaple2.png" alt="API-exmaple2" border="0" width="800">


## Distribute

## Developers

- [VoblaSuperFish](https://github.com/VoblaSuperFish)

## License
The PetProjectForRestoraunt project is distributed under the MIT license.

## Examples of API operation
<p>
      <img src="https://i.ibb.co/HtjTxsX/example1.png" alt="example1" border="0" width="300" height="300">
      <img src="https://i.ibb.co/zZv7MzN/example2.png" alt="example2" border="0" width="300" height="300">
      <img src="https://i.ibb.co/G3zJBDB/example3.png" alt="example3" border="0" width="300" height="300">
</p>
      
